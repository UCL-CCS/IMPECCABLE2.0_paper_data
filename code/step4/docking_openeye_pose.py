...
parser.add_argument(
    "-s", "--smiles", type=Path, required=True, help="Input smiles"
)

parser.add_argument(
    "-r", "--rec", type=Path, required=True, help="receptor file"
)

parser.add_argument(
    "-p", "--prot", type=Path, required=True, help="protein file"
)

parser.add_argument(
    "-o", "--output_traj", type=Path, required=True, help="output_traj"
)
                                                                            
                                                                                      
args = parser.parse_args()

df_smiles = pd.read_csv(args.smiles, sep=' ', names=['smiles', 'id']) # smilesdatafile. smiles column is 'smiles'
recep_file = args.rec#'input/rec_4ui5.oedu' # receptor oedu file for openeye 
...
output_traj = args.output_traj#'output_combined_trajectories' # store pdb + dcd files for complex poses
score_pattern = 'scores/4ui5_scores' #store scores like this (will have #ranks files)
protein_pdb = args.prot#'prot.pdb' #protein pdb file to use to store. Will save everything in this file to complex


### Running docking in parallel
run_mpi_docking(rank, size, df_smiles, recep_file, max_confs, score_cutoff, protein_pdb, temp_dir, output_traj, 'temp')
os.system("sleep 3")
