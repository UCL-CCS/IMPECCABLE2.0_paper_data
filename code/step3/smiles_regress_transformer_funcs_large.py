...
def preprocess_smiles_pair_encoding(data, tokenizer, maxlen):
    ...
    bad_toks = [11, 16, 19, 22, 23, 34, 46, 49, 50, 53, 57, 61, 66, 68, 78, 83, 86, 97, 103, 113, 115, 119, 126, 2083]
    tokenized_data_list = []
    smi_data = []
    for smi in data:
        tok_smi = list(pad(tokenizer(smi)['input_ids'], maxlen, 0))
        if any(x in tok_smi for x in bad_toks):
            #print(smi)
            continue
        else: 
            smi_data.append(smi)
            tokenized_data_list.append(tok_smi)

    tokenized_data = np.array(tokenized_data_list)
    
    return smi_data, tokenized_data


def _preprocess_smiles_pair_encoding(data, tokenizer, maxlen):
...
def split_data_list(hyper_params, size, rank):
    ...
    list_dir_files = np.array_split(np.array(list_dir_files), int(size))[int(rank)]
    return list_dir_files

def large_scale_split(hyper_params, databases, size, rank):
    DATA_FILE_PATH = hyper_params['inference_data']['data_dir']
...
def large_inference_data_gen(hyper_params, tokenizer, dirs, fil, rank):
    ...
    Data_smiles_total = pd.read_csv(data_path_inference)['SMILE']
    #Data_smiles_raw = np.array_split(Data_smiles_total)[rank]
    #del(Data_smiles_total)

    smi_data, x_inference = preprocess_smiles_pair_encoding(Data_smiles_total,
                                                    tokenizer,
                                                    maxlen
                                                    )
    #del(Data_smiles_total)
    return smi_data, x_inference
