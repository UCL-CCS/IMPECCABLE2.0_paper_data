...
def initialize_hvd():
    ...
    if gpus:
        try:
            ...
            print(f"rank:{hvd.rank()}, local:{hvd.local_rank()}")
            import sys
            sys.exit()
    return 
...
def train_val_data(hyper_params):
    ...
    if tokenizer_params['category'] == 'smilespair':
        ...
        try:
            ...
            x_train = np.array([preprocess_spe_one_at_time(str(smi), tokenizer, maxlen) for smi in tqdm(x_smiles_train.values)])
            #x_smiles_train.values.apply(preprocess_spe_one_at_time, args = (tokenizer, maxlen))
            x_val = np.array([preprocess_spe_one_at_time(str(smi), tokenizer, maxlen) for smi in tqdm(x_smiles_val.values)])
        except:
            import sys
...
def _train_val_data(hyper_params):
...
class TrainingAndCallbacks:
    ...
    def training(
        ...
        batch_size = hyper_params['general']['batch_size']
        ...
        while batch_size >= 1:
            try:
                ...
                if 'HorovodAllgather' in str(e):
                    print(f"⚠️ Horovod Allgather error detected!"+\
                                f"Reducing batch size from {batch_size} to {batch_size // 2}")
                    batch_size //= 2  # Reduce batch size by half

                else:
                    raise  # Raise the error if it's something else

        print("❌ Training failed even with minimum batch size.")
        return None

    def _training(self, model, x_train, y_train, validation_data, hyper_params):
